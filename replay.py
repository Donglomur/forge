...
class ReplayScene:
    ...
    def rollout(self, wrist_poses, finger_qpos, obj_pose, substeps=32,
                lift_thresh=0.03, settle=50, overclose=0.5, grip_gain=80.0):
        m, d = self.model, self.data
        for i in range(m.nu):                           # firm holding grip (gate-independent)
    ...
    def grasp_lift(self, wrist_poses, finger_qpos, obj_world_traj, substeps=32,
                   lift_thresh=0.03, settle=120, overclose=0.5, grip_gain=80.0):
        ...
        world poses (used to seat the object at the grasp frame). `grip_gain` sets the
        finger holding stiffness (independent of the force-closure gate)."""
        m, d = self.model, self.data
        for i in range(m.nu):
            m.actuator_gainprm[i, 0] = grip_gain; m.actuator_biasprm[i, 1] = -grip_gain
        mujoco.mj_resetData(m, d)
    ...
    def dynamic_lift(self, wrist_poses, finger_qpos, obj_rel, g, lift_h=0.12,
                     ...
                     mu=1.6, slip_tol=0.015, grip_gain=80.0):
        ...
        `grip_gain` sets the finger actuator stiffness for HOLDING (independent of
        the force-closure gate, which keeps the scene's default gain). Returns dict
        with success, lift, and recorded wrist/finger/object trajs."""
        m, d = self.model, self.data
        for i in range(m.nu):                           # firmer grip force than the gate
            m.actuator_gainprm[i, 0] = grip_gain; m.actuator_biasprm[i, 1] = -grip_gain
    ...
    def pick(self, grasp_q, open_q, obj_xy, tip_names, wrist_R=None,
        ...
        the CLOSED fingertips meet at the object, hovers `approach` m above with a
        SPREAD pre-grasp so the open fingers descend AROUND the object rather than
        through it, closes onto it, lifts straight up, and holds at the top.

        Stability is the whole game here: stiff position fingers squeezing a small
        rigid body will inject energy and the solver ejects the object. Two things
        prevent that, a hard CAP on actuator force (`grip_force`, so the grip can
        physically only push so hard) and extra solver iterations (`solver_iters`).
        Success is judged on the FINAL state, the object held aloft and at rest,
        not on transient contact speeds during the approach.

        Reliable within the hand's pinch aperture (objects too large can't be
        cleared on descent; too small slip through). Returns dict(success,
        final_height, final_speed, end_contacts, object_path).
        """
        m, d = self.model, self.data
        if wrist_R is None: wrist_R = np.eye(3)
        ...
        for i in range(nu):
            m.actuator_gainprm[i, 0] = grip_gain; m.actuator_biasprm[i, 1] = -grip_gain
            m.actuator_forcelimited[i] = 1                   # CAP the grip force
            m.actuator_forcerange[i] = [-grip_force, grip_force]
        for gi in range(m.ngeom):
            if m.geom_bodyid[gi] == self.obj_bid:
                m.geom_friction[gi] = [mu, 0.1, 0.005]       # sliding + torsional + rolling
        obj_gid = [gi for gi in range(m.ngeom) if m.geom_bodyid[gi] == self.obj_bid]
        half = self.obj_half_z
